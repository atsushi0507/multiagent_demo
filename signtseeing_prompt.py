def daytrip_sightseeing_prompt(departure, destination, budget, trip_dates):
    ...
    2. 出発時間、帰路に着く時間はフォーマット例の通りでなくて良いです。
    3. 観光スポットに応じて滞在時間を調整してください。
    4. 目的地では観光スポットと食事の時間を含めたプランを作成。
    5. 食事場所は具体的には提案せず、食事エージェントに任せるため「昼食」「夕食」などのラベルだけ表示。
    6. 食事前後の観光スポットは、周辺に複数の飲食店があるエリアを選定。
    7. 以下のフォーマット例に従って提案してください。
